...
var categories;
var titles = JSON.parse(localStorage.getItem('titles')), dates = [], images = JSON.parse(localStorage.getItem('images')), details = [], links = [], positions = [], checked = JSON.parse(localStorage.getItem('checked'));
var connected;
var intervalpop;
var timeout;
...
for (var i = 0; i < 14; i++) {
	if (checked[i] == 1) {
		$("#content").append(timpTSlide.replace(/{{sectitle}}/g, sectionsTitles[i]).replace(/{{i}}/g, i));
		activeCount += 1;
		positions.push(i);
	}
}
...
function onDeviceReady() {
	...
	$('.closesettings').click(function() {
		localStorage.setItem('layout2instructions', 1);
		$('.instructions').hide();
	});
...
function onBackKeyDown() {
	...
	if(page == 'layout2.html' || page == 'index.html') {
...
function loadAD() {
	...
	if (connected == 1) {
		/*$.ajax({
			...
			if (response.ad == true) {
				admob.initAdmob('ca-app-pub-1333731159795332/4974164791', 'ca-app-pub-1333731159795332/6450897994');
